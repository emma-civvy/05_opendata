...
fe_month_sum <- fe_month %>% 
  ...
  summarize(across(.cols = c(dayl.s,
                             srad.wm2,
                             tmax.c,
                             tmin.c,
                             vp.pa
...
finalplots <- fe_month_sum %>% 
  pivot_longer(mean_dayl.s:sum_prcp.mm) %>% ## to group by/nest weather factors, need to have single column
  group_by(name) %>% 
  nest() %>% ## look at datafrme at this point
